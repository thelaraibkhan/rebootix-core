...
import type { RebootixConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("rebootix-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("rebootix-bin-sherpa-");
      const modelDir = await makeTempDir("rebootix-sherpa-model-");
      ...
      const cfg: RebootixConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("rebootix-bin-whispercpp-");
      const modelDir = await makeTempDir("rebootix-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("rebootix-bin-gemini-");
      ...
      const cfg: RebootixConfig = { tools: { media: { image: {} } } };
