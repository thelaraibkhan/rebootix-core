...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("REBOOTIX_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.REBOOTIX_STATE_DIR = path.join(base, ".rebootix");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "rebootix-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".rebootix", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
