...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `rebootix-e2e-${name}-`));
  const configDir = path.join(homeDir, ".rebootix");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          REBOOTIX_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        REBOOTIX_GATEWAY_TOKEN: inst.gatewayToken,
        REBOOTIX_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          REBOOTIX_GATEWAY_PORT: String(gwA.port),
          REBOOTIX_GATEWAY_TOKEN: gwA.gatewayToken,
          REBOOTIX_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          REBOOTIX_GATEWAY_PORT: String(gwB.port),
          REBOOTIX_GATEWAY_TOKEN: gwB.gatewayToken,
          REBOOTIX_GATEWAY_PASSWORD: "",
        }),
