...
This document describes how the Rebootix ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`rebootix acp` exposes an ACP agent over stdio and forwards prompts to a running
Rebootix Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a Rebootix Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `rebootix acp` over stdio.

Example config:

```bash
rebootix config set gateway.remote.url wss://gateway-host:18789
rebootix config set gateway.remote.token <token>
```

Example run:

```bash
rebootix acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
rebootix acp --session agent:main:main
rebootix acp --session agent:design:main
rebootix acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Rebootix ACP” to start a thread.

## Execution Model

- ACP client spawns `rebootix acp` and speaks ACP messages over stdio.
...
```bash
rebootix acp --session agent:main:main
rebootix acp --session-label "support inbox"
rebootix acp --reset-session
```
...
`rebootix acp` resolves the Gateway URL and auth from CLI flags or config:
