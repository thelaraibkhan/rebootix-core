...
read_when:
  - You are building a Rebootix plugin
  - You need to ship a plugin config schema or debug plugin validation errors
---

# Plugin manifest (rebootix.plugin.json)

Every plugin **must** ship a `rebootix.plugin.json` file in the **plugin root**.
Rebootix uses this manifest to validate configuration **without executing plugin
