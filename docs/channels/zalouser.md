...
read_when:
  - Setting up Zalo Personal for Rebootix
...
- Install via CLI: `rebootix plugins install @rebootix/zalouser`
- Or from a source checkout: `rebootix plugins install ./extensions/zalouser`
...
- `rebootix pairing list zalouser`
- `rebootix pairing approve zalouser <code>`
...
- On startup, Rebootix resolves group/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
...
- Re-login: `rebootix channels logout --channel zalouser && rebootix channels login --channel zalouser`
