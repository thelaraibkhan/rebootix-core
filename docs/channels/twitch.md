...
read_when:
  - Setting up Twitch chat integration for Rebootix
---

# Twitch (plugin)

Twitch chat support via IRC connection. Rebootix connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
rebootix plugins install @rebootix/twitch
```

Local checkout (when running from a git repo):

```bash
rebootix plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `REBOOTIX_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "rebootix", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use REBOOTIX_TWITCH_ACCESS_TOKEN env var)
...
```bash
REBOOTIX_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
