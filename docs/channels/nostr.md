...
read_when:
  - You want Rebootix to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Rebootix to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`rebootix onboard`) and `rebootix channels add` list optional channel plugins.
...
```bash
rebootix plugins install @rebootix/nostr
```

Use a local checkout (dev workflows):

```bash
rebootix plugins install --link <path-to-rebootix>/extensions/nostr
```
...
```json
{
  "channels": {
    "nostr": {
      ...
      "profile": {
        "name": "rebootix",
        "displayName": "Rebootix",
        "about": "Personal assistant DM bot",
        "picture": "https://example.com/avatar.png",
        "banner": "https://example.com/banner.png",
        "website": "https://example.com",
        "nip05": "rebootix@example.com",
        "lud16": "rebootix@example.com"
      }
    }
  }
}
