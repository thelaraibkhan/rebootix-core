...
```bash
rebootix plugins install @rebootix/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
rebootix plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "Rebootix" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `rebootix pairing list nextcloud-talk`
  - `rebootix pairing approve nextcloud-talk <CODE>`
