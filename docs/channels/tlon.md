...
Tlon is a decentralized messenger built on Urbit. Rebootix connects to your Urbit ship and can
...
```bash
rebootix plugins install @rebootix/tlon
```

Local checkout (when running from a git repo):

```bash
rebootix plugins install ./extensions/tlon
```
...
Use these with `rebootix message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Rebootix replies in-thread.
