...
Matrix is an open, decentralized messaging protocol. Rebootix connects as a Matrix **user**
...
```bash
rebootix plugins install @rebootix/matrix
```

Local checkout (when running from a git repo):

```bash
rebootix plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `rebootix plugins install @rebootix/matrix`
   - From a local checkout: `rebootix plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Rebootix calls the same
     login endpoint, stores the access token in `~/.rebootix/credentials/matrix/credentials.json`,
...
- On first connection, Rebootix requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Rebootix logs a warning.
...
`~/.rebootix/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `rebootix pairing list matrix`
  - `rebootix pairing approve matrix <CODE>`
...
- On startup, Rebootix resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
