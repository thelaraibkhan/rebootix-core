...
read_when:
  - Designing workspace memory (~/.rebootix/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Rebootix integration
  - Adding offline recall + reflection (retain/recall/reflect)
---

# Workspace Memory v2 (offline): research notes

Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.rebootix/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.rebootix/workspace` as canonical human-readable memory.
...
```
~/.rebootix/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Rebootix**, but keep a separable core library.

### Why integrate into Rebootix?

- Rebootix already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `rebootix memory recall "…" --k 25 --since 30d`
  - `rebootix memory reflect --since 7d`
...
Pragmatic take for `~/.rebootix/workspace`:
