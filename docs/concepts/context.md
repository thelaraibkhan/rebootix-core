...
read_when:
  - You want to understand what “context” means in Rebootix
...
“Context” is **everything Rebootix sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (Rebootix-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How Rebootix builds the system prompt

The system prompt is **Rebootix-owned** and rebuilt each run. It includes:
...
By default, Rebootix injects a fixed set of workspace files (if present):
