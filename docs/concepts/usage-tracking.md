...
- `/usage cost` in chats: local cost summary aggregated from Rebootix session logs.
- CLI: `rebootix status --usage` prints a full per-provider breakdown.
- CLI: `rebootix channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
