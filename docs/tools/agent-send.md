---
summary: "Direct `rebootix agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
---

# `rebootix agent` (direct agent runs)

`rebootix agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `rebootix message --target`).
...
```bash
rebootix agent --to +15555550123 --message "status update"
...
rebootix agent --to +15555550123 --message "Summon reply" --deliver
