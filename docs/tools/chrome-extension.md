---
summary: "Chrome extension: let Rebootix drive your existing Chrome tab"
read_when:
...
The Rebootix Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate rebootix-managed Chrome profile.
...
Rebootix then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Rebootix release (npm package) as static files. There is no separate “build” step.

After upgrading Rebootix:

- Re-run `rebootix browser extension install` to refresh the installed files under your Rebootix state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Rebootix ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Rebootix to control.
...
- `ON`: attached; Rebootix can drive that tab.
...
Debugging: `rebootix sandbox explain`
...
`rebootix browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `rebootix browser extension install` first to copy the extension to a stable location under your Rebootix state directory.
...
- **This is not isolated** like the dedicated rebootix-managed profile.
