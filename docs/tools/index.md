---
summary: "Agent tool surface for Rebootix (browser, canvas, nodes, message, cron) replacing legacy `rebootix-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `rebootix-*` skills
---

# Tools (Rebootix)

Rebootix exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `rebootix-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `rebootix.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Rebootix logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`rebootix node run`).
...
- Requires a Brave API key (recommended: `rebootix configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated Rebootix-managed browser.
...
- Quick smoke: `rebootix nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `rebootix gateway` restart in-place)
