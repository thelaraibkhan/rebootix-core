...
- **gateway host** → `rebootix` process on the gateway machine
...
`~/.rebootix/exec-approvals.json` directly.

CLI: `rebootix approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
