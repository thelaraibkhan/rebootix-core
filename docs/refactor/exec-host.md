...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.rebootix/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.rebootix/node.json` (node id + pairing token).
...
Path: `~/.rebootix/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.rebootix/exec-approvals.sock",
...
- Unix socket at `~/.rebootix/exec-approvals.sock` (0600).
