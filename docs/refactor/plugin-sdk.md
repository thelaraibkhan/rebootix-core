...
- Publish as `rebootix/plugin-sdk` (or export from core under `rebootix/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `RebootixPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: RebootixConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: RebootixConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: RebootixConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: RebootixConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: RebootixConfig): string;
  };
};
```
...
- Introduce `rebootix/plugin-sdk`.
- Add `api.runtime` to `RebootixPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `rebootixRuntime: ">=2026.2.0"`).
