...
```bash
rebootix tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `rebootix tui --deliver`
...
- Check the Gateway logs: `rebootix logs --follow`.
- Confirm the agent can run: `rebootix status` and `rebootix models status`.
...
- No agents in picker: check `rebootix agents list` and your routing config.
