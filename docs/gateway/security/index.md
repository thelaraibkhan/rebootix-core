...
## Quick check: `rebootix security audit`
...
- Tighten local perms (`~/.rebootix` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).

Running an AI agent with shell access on your machine is... _spicy_. Here’s how to not get pwned.

Rebootix is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
If you run `--deep`, Rebootix also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.rebootix/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile`
- **Discord bot token**: config/env (token file not yet supported)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**: `~/.rebootix/credentials/<channel>-allowFrom.json`
- **Model auth profiles**: `~/.rebootix/agents/<agentId>/agent/auth-profiles.json`
- **Legacy OAuth import**: `~/.rebootix/credentials/oauth.json`
...
`rebootix security audit` warns when this setting is enabled.
...
gateway:
  ...
  auth:
    mode: password
    password: ${REBOOTIX_GATEWAY_PASSWORD}
```
...
Rebootix stores session transcripts on disk under `~/.rebootix/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.rebootix` (see the audit section below). If you need
...
Rebootix can refresh the skills list mid-session:
...
- If you install plugins from npm (`rebootix plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.rebootix/extensions/<pluginId>/` (or `$REBOOTIX_STATE_DIR/extensions/<pluginId>/`).
  - Rebootix uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
rebootix pairing approve <channel> <code>
```
...
By default, Rebootix routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
Rebootix has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.rebootix/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.rebootix or your logs.”
...
4. **Re-run audit**
   - `rebootix security audit --deep` and confirm the report is clean.
...
- `~/.rebootix/rebootix.json`: `600` (user read/write only)
- `~/.rebootix`: `700` (user only)

`rebootix doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `REBOOTIX_GATEWAY_PORT`
...
The Gateway broadcasts its presence via mDNS (`_rebootix-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `REBOOTIX_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `rebootix doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `REBOOTIX_GATEWAY_PASSWORD`).

Rotation checklist (token/password):

1. Generate/set a new secret (`gateway.auth.token` or `REBOOTIX_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), Rebootix
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. Rebootix verifies the identity by resolving the
...
- Rebootix will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.rebootix/` (or `$REBOOTIX_STATE_DIR/`) may contain secrets or private data:

- `rebootix.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `rebootix status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@rebootix", "@mybot"] }
      }
    ]
  }
}
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.rebootix/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `rebootix` profile).
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `rebootix gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `REBOOTIX_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/rebootix/rebootix-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.rebootix/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).

### Collect for a report

- Timestamp, gateway host OS + Rebootix version
...
Found a vulnerability in Rebootix? Please report responsibly:

1. Email: security@rebootix.ai
