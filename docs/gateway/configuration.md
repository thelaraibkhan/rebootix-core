---
summary: "All configuration options for ~/.rebootix/rebootix.json with examples"
read_when:
...
Rebootix reads an optional **JSON5** config from `~/.rebootix/rebootix.json` (comments + trailing commas allowed).

If the file is missing, Rebootix uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.rebootix/workspace`). You usually only need a config to:
...
Rebootix only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `rebootix doctor`, `rebootix logs`, `rebootix health`, `rebootix status`, `rebootix service`, `rebootix help`).
- Run `rebootix doctor` to see the exact issues.
- Run `rebootix doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `rebootix config set`. Keep a backup of `~/.rebootix/rebootix.json`.
...
```bash
rebootix gateway call config.get --params '{}' # capture payload.hash
rebootix gateway call config.apply --params '{
...
```bash
rebootix gateway call config.get --params '{}' # capture payload.hash
rebootix gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.rebootix/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@rebootix", "reisponde"] },
      },
    ],
  },
  channels: {
...
```json5
// ~/.rebootix/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [{ id: "main", workspace: "~/.rebootix/workspace" }],
}
```
...
{ "$include": "/etc/rebootix/base.json5" } // absolute
...
```json5
// ~/.rebootix/clients/mueller/agents.json5
[
...
```json5
// ~/.rebootix/clients/mueller/broadcast.json5
{
...
Rebootix reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, Rebootix runs your login shell and imports only the missing expected keys (never overrides).
...
- `REBOOTIX_LOAD_SHELL_ENV=1`
- `REBOOTIX_SHELL_ENV_TIMEOUT_MS=15000`
...
Rebootix stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.rebootix/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.rebootix/credentials/oauth.json` (or `$REBOOTIX_STATE_DIR/credentials/oauth.json`)
...
- `~/.rebootix/agent/*` (migrated by `rebootix doctor` into `~/.rebootix/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `REBOOTIX_OAUTH_DIR`
- Agent dir (default agent root override): `REBOOTIX_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, Rebootix imports `oauth.json` entries into `auth-profiles.json`.
...
If set, Rebootix derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/rebootix/rebootix-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/rebootix/rebootix.log`.
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/rebootix/rebootix.log",
    consoleLevel: "info",
    consoleStyle: "pretty",
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.rebootix/credentials/whatsapp/biz
          // authDir: "~/.rebootix/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- The legacy single-account Baileys auth dir is migrated by `rebootix doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@rebootix", "rebootix"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@rebootix"],
        },
      },
    ],
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.rebootix/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.rebootix/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.rebootix/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.rebootix/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.rebootix/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.rebootix/workspace-home" },
      { id: "work", workspace: "~/.rebootix/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `rebootix.json`).
...
Rebootix starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["rebootix-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
          // guild id (preferred) or slug
          slug: "friends-of-rebootix",
...
Rebootix starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `rebootix plugins install @rebootix/mattermost` (or `./extensions/mattermost` from a git checkout).
...
Rebootix starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
Rebootix spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, Rebootix
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/rebootix" } },
}
```
...
When a file exceeds this limit, Rebootix logs a warning and injects a truncated
...
message envelopes). If unset, Rebootix uses the host timezone at runtime.
...
`[rebootix]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[rebootix]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[rebootix]"`, Rebootix will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, Rebootix generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.rebootix/settings/tts.json",
      elevenlabs: {
...
Rebootix also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, Rebootix currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `rebootix configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:rebootix`: all built-in Rebootix tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.rebootix/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.rebootix/sandboxes",
        docker: {
          image: "rebootix-sandbox:bookworm-slim",
          containerPrefix: "rebootix-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "rebootix-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "rebootix-sandbox-browser:bookworm-slim",
          containerPrefix: "rebootix-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
Rebootix uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.rebootix/agents/<agentId>/agent/models.json` or by defining the same schema inside your
Rebootix config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, Rebootix writes/merges a `models.json` into
`~/.rebootix/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. Rebootix uses
...
- Shortcut: `rebootix onboard --auth-choice opencode-zen`.
...
Shortcut: `rebootix onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `rebootix onboard --auth-choice moonshot-api-key`.
...
- Set `KIMI_API_KEY` in the environment or use `rebootix onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `rebootix onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `rebootix onboard --auth-choice minimax-api`.
...
- Override the agent config root with `REBOOTIX_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.rebootix/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.rebootix/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.rebootix/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, Rebootix stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.rebootix/skills` (workspace skills
...
from `~/.rebootix/extensions`, `<workspace>/.rebootix/extensions`, plus any
...
### `browser` (rebootix-managed browser)

Rebootix can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for rebootix and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (Rebootix.app menubar, or `rebootix gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      rebootix: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/rebootix" }
...
- Examples: `"/ui"`, `"/rebootix"`, `"/apps/rebootix"`.
...
- When a connection comes from one of these IPs, Rebootix uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `rebootix gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `REBOOTIX_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `REBOOTIX_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. Rebootix
...
- Rebootix.app watches `~/.rebootix/rebootix.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.rebootix/rebootix.json` (or `REBOOTIX_CONFIG_PATH`) and applies changes automatically.
...
- `~/.rebootix/rebootix.json` (or `REBOOTIX_CONFIG_PATH`)
...
- `REBOOTIX_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `rebootix --dev …` → uses `~/.rebootix-dev` + shifts ports from base `19001`
- `rebootix --profile <name> …` → uses `~/.rebootix-<name>` (port via config/env/flags)
...
```bash
REBOOTIX_CONFIG_PATH=~/.rebootix/a.json \
REBOOTIX_STATE_DIR=~/.rebootix-a \
rebootix gateway --port 19001
```
...
Gmail helper config (used by `rebootix webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "rebootix@gmail.com",
...
- Set `REBOOTIX_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, Rebootix defaults `serve.path` to `/` so
...
Default root: `~/.rebootix/workspace/canvas`  
Default port: `18793` (chosen to avoid the rebootix browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__rebootix__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__rebootix__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
{
  canvasHost: {
    root: "~/.rebootix/workspace/canvas",
    port: 18793,
    liveReload: true,
  },
}
...
- env: `REBOOTIX_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.rebootix/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.rebootix/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.rebootix/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_rebootix-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `rebootix` (advertises `rebootix.local`). Override with `REBOOTIX_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_rebootix-gw._tcp` under `~/.rebootix/dns/` using the configured discovery domain (example: `rebootix.internal.`).
