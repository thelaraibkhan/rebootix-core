...
- Replaces the legacy `gateway` command. CLI entry point: `rebootix gateway`.
...
rebootix gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```

- Config hot reload watches `~/.rebootix/rebootix.json` (or `REBOOTIX_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__rebootix__/canvas/` from `~/.rebootix/workspace/canvas`. Disable with `canvasHost.enabled=false` or `REBOOTIX_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `REBOOTIX_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `REBOOTIX_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.rebootix.*` may still exist)
- Linux: `rebootix-gateway-<profile>.service`
- Windows: `Rebootix Gateway (<profile>)`

Install metadata is embedded in the service config:

- `REBOOTIX_SERVICE_MARKER=rebootix`
- `REBOOTIX_SERVICE_KIND=gateway`
- `REBOOTIX_SERVICE_VERSION=<version>`
...
```bash
rebootix --dev setup
rebootix --dev gateway --allow-unconfigured
...
- `REBOOTIX_STATE_DIR=~/.rebootix-dev`
- `REBOOTIX_CONFIG_PATH=~/.rebootix-dev/rebootix.json`
- `REBOOTIX_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.rebootix/workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):

- Base port = `gateway.port` (or `REBOOTIX_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `REBOOTIX_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:

- unique `gateway.port`
- unique `REBOOTIX_CONFIG_PATH`
- unique `REBOOTIX_STATE_DIR`
...
```bash
rebootix --profile main gateway install
rebootix --profile rescue gateway install
```

Example:

```bash
REBOOTIX_CONFIG_PATH=~/.rebootix/a.json REBOOTIX_STATE_DIR=~/.rebootix-a rebootix gateway --port 19001
REBOOTIX_CONFIG_PATH=~/.rebootix/b.json REBOOTIX_STATE_DIR=~/.rebootix-b rebootix gateway --port 19002
```
...
- `health` — full health snapshot (same shape as `rebootix health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `rebootix`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `rebootix gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.rebootix.*` is cleaned up).
  - `rebootix doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are Rebootix profile services.
  ...
  - Cleanup: `rebootix gateway uninstall` (current service) and `rebootix doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `rebootix gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- Rebootix.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.rebootix.*` labels still unload cleanly).
- To stop it cleanly, use `rebootix gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `rebootix gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `rebootix gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

Rebootix installs a **systemd user service** by default on Linux/WSL2. We
...
`rebootix gateway install` writes the user unit. `rebootix doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/rebootix-gateway[-<profile>].service`:

```
[Unit]
Description=Rebootix Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/rebootix gateway --port 18789
Restart=always
RestartSec=5
Environment=REBOOTIX_GATEWAY_TOKEN=
...
```
systemctl --user enable --now rebootix-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/rebootix-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:

```
sudo systemctl daemon-reload
sudo systemctl enable --now rebootix-gateway[-<profile>].service
```
...
- `rebootix gateway health|status` — request health/status over the Gateway WS.
- `rebootix message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `rebootix agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `rebootix gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `rebootix gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `rebootix gateway` and the legacy TCP control port.
