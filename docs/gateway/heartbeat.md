...
- During heartbeat runs, Rebootix treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `rebootix sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Rebootix skips the heartbeat run entirely (no model call).
...
```bash
rebootix system event --text "Check for urgent follow-ups" --mode now
```
