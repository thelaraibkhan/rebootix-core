---
summary: "Quick troubleshooting guide for common Rebootix failures"
read_when:
...
When Rebootix misbehaves, here's how to fix it.
...
| ---------------------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------- |
| `rebootix status`                  | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview                       |
| `rebootix status --all`            | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail                                   | When you need to share a debug report             |
| `rebootix status --deep`           | Runs gateway health checks (incl. provider probes; requires reachable gateway)                         | When “configured” doesn’t mean “working”          |
| `rebootix gateway probe`           | Gateway discovery + reachability (local + remote targets)                                              | When you suspect you’re probing the wrong gateway |
| `rebootix channels status --probe` | Asks the running gateway for channel status (and optionally probes)                                    | When gateway is reachable but channels misbehave  |
| `rebootix gateway status`          | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error                      | When the service “looks loaded” but nothing runs  |
| `rebootix logs --follow`           | Live logs (best signal for runtime issues)                                                             | When you need the actual failure reason           |

**Sharing output:** prefer `rebootix status --all` (it redacts tokens). If you paste `rebootix status`, consider setting `REBOOTIX_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  rebootix models auth setup-token --provider anthropic
  ```
...
```bash
rebootix models auth paste-token --provider anthropic
rebootix models status
```
...
- Preferred: `rebootix logs --follow`
- File logs (always): `/tmp/rebootix/rebootix-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$REBOOTIX_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u rebootix-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ```bash
  rebootix config set gateway.mode local
  ```
...
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  rebootix config set gateway.mode remote
  rebootix config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `rebootix setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.rebootix/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `rebootix doctor`
...
- then run `rebootix sandbox recreate --agent <id>` (or `--all`)
...
- `gateway.mode` must be `local` for `rebootix gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `rebootix gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `rebootix gateway status` and `rebootix doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `REBOOTIX_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `rebootix gateway status` shows a config mismatch**
...
- Fix: rerun `rebootix gateway install --force` from the same `--profile` / `REBOOTIX_STATE_DIR` you want the service to use.

**If `rebootix gateway status` reports service config issues**

- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `rebootix doctor` to update it (or `rebootix gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `REBOOTIX_GATEWAY_TOKEN`) and restart the service.

**If `rebootix gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/rebootix` on disk.
...
Symptoms: `pwd` or file tools show `~/.rebootix/sandboxes/...` even though you
...
Rebootix intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `rebootix models list` or
  `rebootix models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${REBOOTIX_CONFIG_PATH:-$HOME/.rebootix/rebootix.json}"
```
...
tail -f "$(ls -t /tmp/rebootix/rebootix-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
rebootix logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.rebootix/agents/<agentId>/sessions/
```
...
rebootix logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
rebootix gateway --verbose
```

If you’re logged out / unlinked:

```bash
rebootix channels logout
trash "${REBOOTIX_STATE_DIR:-$HOME/.rebootix}/credentials" # if logout can't cleanly remove everything
rebootix channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/rebootix/rebootix-*.log | head -1)" | tail -20
```

### High Memory Usage

Rebootix keeps conversation history in memory.
...
Rebootix now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
- `rebootix doctor` reports every invalid entry.
- `rebootix doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `rebootix logs`, `rebootix health`, `rebootix status`, `rebootix gateway status`, and `rebootix gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/rebootix/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `rebootix models status` (CLI).
...
5. Run `rebootix channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. Rebootix scrubs/normalizes tool
...
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/rebootix/rebootix/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
# Or: launchctl bootout gui/$UID/bot.molt.gateway (replace with bot.molt.<profile>; legacy com.rebootix.* still works)
```
...
Ensure the global `rebootix` CLI is installed and matches the app version:

```bash
rebootix --version
npm install -g rebootix@<version>
```
...
#   ${REBOOTIX_CONFIG_PATH:-$HOME/.rebootix/rebootix.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
rebootix gateway --verbose
rebootix channels login --verbose
```
...
| Log                               | Location                                                                                                                                                                                                                                                                                                                    |
| --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gateway file logs (structured)    | `/tmp/rebootix/rebootix-YYYY-MM-DD.log` (or `logging.file`)                                                                                                                                                                                                                                                                 |
| Gateway service logs (supervisor) | macOS: `$REBOOTIX_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.rebootix/logs/...`; profiles use `~/.rebootix-<profile>/logs/...`)<br />Linux: `journalctl --user -u rebootix-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "Rebootix Gateway (<profile>)" /V /FO LIST` |
| Session files                     | `$REBOOTIX_STATE_DIR/agents/<agentId>/sessions/`                                                                                                                                                                                                                                                                            |
| Media cache                       | `$REBOOTIX_STATE_DIR/media/`                                                                                                                                                                                                                                                                                                |
| Credentials                       | `$REBOOTIX_STATE_DIR/credentials/`                                                                                                                                                                                                                                                                                          |
...
tail -20 /tmp/rebootix/rebootix-*.log
```
...
trash "${REBOOTIX_STATE_DIR:-$HOME/.rebootix}"
rebootix channels login         # re-pair WhatsApp
rebootix gateway restart           # or: rebootix gateway
```
...
1. Check logs first: `/tmp/rebootix/` (default: `rebootix-YYYY-MM-DD.log`, or your configured `logging.file`)
