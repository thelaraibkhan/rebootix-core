...
This guide summarizes a sane workflow for working on the pi integration in Rebootix.
...
- Trigger the agent directly:
  - `pnpm rebootix agent --message "Hello" --thinking low`
...
State lives under the Rebootix state directory. Default is `~/.rebootix`. If `REBOOTIX_STATE_DIR` is set, use that directory instead.

To reset everything:

- `rebootix.json` for config
...
- https://docs.rebootix.ai/testing
- https://docs.rebootix.ai/start/getting-started
