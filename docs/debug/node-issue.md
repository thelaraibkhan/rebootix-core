...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Rebootix
---
...
Running Rebootix via Node with `tsx` fails at startup with:

```
[rebootix] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `rebootix status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch rebootix.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node rebootix.mjs status` works on Node 25.
