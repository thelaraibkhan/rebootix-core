...
Deploy Rebootix on Railway with a one-click template and finish setup in your browser.
...
<a href="https://railway.com/deploy/rebootix-railway-template" target="_blank" rel="noreferrer">
  Deploy on Railway
</a>
...
- `https://<your-railway-domain>/rebootix` — Control UI

## What you get

- Hosted Rebootix Gateway + Control UI
...
- `REBOOTIX_STATE_DIR=/data/.rebootix` (recommended)
- `REBOOTIX_WORKSPACE_DIR=/data/workspace` (recommended)
- `REBOOTIX_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your Rebootix state + workspace so you can migrate to another host without losing config or memory.
