---
summary: "Use Venice AI privacy-focused models in Rebootix"
read_when:
  - You want privacy-focused inference in Rebootix
...
```bash
rebootix onboard --auth-choice venice-api-key
```
...
```bash
rebootix chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Rebootix shows all available Venice models. Pick based on your needs:
...
```bash
rebootix models set venice/claude-opus-45
rebootix models set venice/llama-3.3-70b
```

List all available models:

```bash
rebootix models list | grep venice
```

## Configure via `rebootix configure`

1. Run `rebootix configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Rebootix automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
rebootix chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
rebootix chat --model venice/claude-opus-45

# Use uncensored model
rebootix chat --model venice/venice-uncensored

# Use vision model with image
rebootix chat --model venice/qwen3-vl-235b-a22b

# Use coding model
rebootix chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `rebootix models list` to see currently available models. Some models may be temporarily offline.
