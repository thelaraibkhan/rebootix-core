---
summary: "Model providers (LLMs) supported by Rebootix"
read_when:
...
Rebootix can use many LLM providers. Pick one, authenticate, then set the default
...
1. Authenticate with the provider (usually via `rebootix onboard`).
