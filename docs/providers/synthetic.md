---
summary: "Use Synthetic's Anthropic-compatible API in Rebootix"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Rebootix registers it as the
...
```bash
rebootix onboard --auth-choice synthetic-api-key
```
...
Note: Rebootix's Anthropic client appends `/v1` to the base URL, so use
