...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `rebootix sandbox explain`.
...
Legacy `agent.*` configs are migrated by `rebootix doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=rebootix-sbx-"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${REBOOTIX_STATE_DIR:-$HOME/.rebootix}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
