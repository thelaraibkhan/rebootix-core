---
summary: "First-run onboarding flow for Rebootix (macOS app)"
read_when:
...
- Writes credentials to `~/.rebootix/credentials/oauth.json`
...
The app can install the global `rebootix` CLI via npm/pnpm so terminal
...
On the first agent run, Rebootix bootstraps a workspace (default `~/.rebootix/workspace`):
...
```bash
rebootix webhooks gmail setup --account you@gmail.com
```
...
- `~/.rebootix/credentials/oauth.json`
