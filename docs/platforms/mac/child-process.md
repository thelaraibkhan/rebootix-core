...
service via the external `rebootix` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`REBOOTIX_PROFILE`; legacy `com.rebootix.*` is supported).
