---
summary: "Rebootix on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Rebootix on a Raspberry Pi
  - Running Rebootix on ARM devices
  - Building a cheap always-on personal AI
---

# Rebootix on Raspberry Pi

## Goal

Run a persistent, always-on Rebootix Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Rebootix features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled rebootix

# Enable if not
sudo systemctl enable rebootix

# Start on boot
sudo systemctl start rebootix
```
...
```bash
# Check logs
journalctl -u rebootix --no-pager -n 100

# Common fix: rebuild
cd ~/rebootix  # if using hackable install
npm run build
sudo systemctl restart rebootix
```
