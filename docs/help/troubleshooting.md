...
You can also set `REBOOTIX_VERBOSE=1` instead of the flag.
...
### `docs.rebootix.ai` shows an SSL error (Comcast/Xfinity)

Some Comcast/Xfinity connections block `docs.rebootix.ai` via Xfinity Advanced Security.
Disable Advanced Security or add `docs.rebootix.ai` to the allowlist, then retry.
...
- Check the allowlist: `rebootix config get agents.defaults.models`
...
If you can, include the relevant log tail from `rebootix logs --follow`.
