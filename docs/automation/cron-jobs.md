...
- Jobs persist under `~/.rebootix/cron/` so restarts don’t lose schedules.
...
rebootix cron list
rebootix cron run <job-id> --force
rebootix cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.rebootix/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `rebootix cron add/edit` or the cron
...
- Job store: `~/.rebootix/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.rebootix/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
rebootix cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
rebootix cron edit <jobId> --agent ops
rebootix cron edit <jobId> --clear-agent
```
...
```bash
rebootix system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`rebootix system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `REBOOTIX_SKIP_CRON`.
