...
Deploy Rebootix on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
<a
  href="https://render.com/deploy?repo=https://github.com/rebootix/rebootix"
  target="_blank"
  rel="noreferrer"
>
  Deploy to Render
...
repository configures everything needed to run Rebootix:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: REBOOTIX_STATE_DIR
        value: /data/.rebootix
      - key: REBOOTIX_WORKSPACE_DIR
        value: /data/workspace
      - key: REBOOTIX_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/rebootix`.
...
If you use the original Rebootix repository, Render will not auto-deploy your Rebootix. To update it, run a manual Blueprint sync from the dashboard.
...
For Rebootix, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Rebootix host.
