...
Rebootix features that can generate provider usage or paid API calls.
...
- `rebootix status --usage` and `rebootix channels list` show provider **usage windows**
...
Rebootix can pick up credentials from:
...
`rebootix models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
