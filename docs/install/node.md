...
read_when:
  - "You installed Rebootix but `rebootix` is “command not found”"
...
Rebootix’s runtime baseline is **Node 22+**.

If you can run `npm install -g rebootix@latest` but later see `rebootix: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
