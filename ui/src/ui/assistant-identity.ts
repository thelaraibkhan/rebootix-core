...
declare global {
  interface Window {
    __REBOOTIX_ASSISTANT_NAME__?: string;
    __REBOOTIX_ASSISTANT_AVATAR__?: string;
  }
}
...
export function resolveInjectedAssistantIdentity(): AssistantIdentity {
  ...
  return normalizeAssistantIdentity({
    name: window.__REBOOTIX_ASSISTANT_NAME__,
    avatar: window.__REBOOTIX_ASSISTANT_AVATAR__,
  });
}
