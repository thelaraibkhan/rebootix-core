...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/rebootix/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/rebootix");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/rebootix/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__REBOOTIX_CONTROL_UI_BASE_PATH__ = "/rebootix";
    const app = mountApp("/rebootix/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/rebootix");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/rebootix/sessions");
  });
