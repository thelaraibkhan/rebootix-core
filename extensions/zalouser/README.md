...
Rebootix extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
rebootix channels login --channel zalouser              # Login via QR
rebootix channels login --channel zalouser --account work
...
```bash
rebootix directory self --channel zalouser
rebootix directory peers list --channel zalouser --query "name"
rebootix directory groups list --channel zalouser --query "work"
rebootix directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
rebootix message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
rebootix message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `rebootix channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
rebootix channels login --channel zalouser --account work
rebootix message send --channel zalouser --account work --target <id> --message "Hello"
