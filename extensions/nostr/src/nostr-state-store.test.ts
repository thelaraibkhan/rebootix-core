...
import type { PluginRuntime } from "rebootix/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.REBOOTIX_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "rebootix-nostr-"));
  process.env.REBOOTIX_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        ...
        return path.join(homedir(), ".rebootix");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.REBOOTIX_STATE_DIR = previous;
    }
