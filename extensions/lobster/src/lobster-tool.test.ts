...
import type { RebootixPluginApi, RebootixPluginToolContext } from "../../../src/plugins/types.js";
import { createLobsterTool } from "./lobster-tool.js";

async function writeFakeLobsterScript(scriptBody: string, prefix = "rebootix-lobster-plugin-") {
...
function fakeApi(): RebootixPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<RebootixPluginToolContext> = {}): RebootixPluginToolContext {
  return {
    config: {} as any,
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: RebootixPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
