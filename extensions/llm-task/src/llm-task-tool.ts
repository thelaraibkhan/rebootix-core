...
// NOTE: This extension is intended to be bundled with Rebootix.
// When running from source (tests/dev), Rebootix internals live under src/.
...
export function createLlmTaskTool(api: RebootixPluginApi) {
  return {
    name: "llm-task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via rebootix.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "rebootix-llm-task-"));
