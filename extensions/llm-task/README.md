...
`rebootix.invoke --each`) without adding new Rebootix code per workflow.
...
This extension depends on Rebootix internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Rebootix extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.rebootix/extensions` as a standalone plugin directory.
