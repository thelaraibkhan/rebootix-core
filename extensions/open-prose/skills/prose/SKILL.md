...
metadata: { "rebootix": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Rebootix `sessions_spawn`
- **File I/O** == Rebootix `read`/`write`
- **Remote fetch** == Rebootix `web_fetch` (or `exec` with curl when POST is required)
