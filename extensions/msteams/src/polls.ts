...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["rebootixPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["rebootix", "pollId"]) ??
    readNestedString(value, ["rebootix", "poll", "id"]) ??
    readNestedString(value, ["data", "rebootixPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "rebootix", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          rebootixPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "rebootix poll vote",
          displayText: "Vote recorded",
          value: { rebootixPollId: pollId, pollId },
        },
      },
    ],
  };
