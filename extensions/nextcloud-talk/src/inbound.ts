...
export async function handleNextcloudTalkInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as RebootixConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as RebootixConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as RebootixConfig);
