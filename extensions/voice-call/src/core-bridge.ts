...
function resolveRebootixRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.REBOOTIX_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set REBOOTIX_ROOT to the package root.");
}

async function importCoreModule<T>(relativePath: string): Promise<T> {
  const root = resolveRebootixRoot();
