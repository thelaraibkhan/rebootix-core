/**
 * Twitch channel plugin for Rebootix.
 *
...
import { buildChannelConfigSchema } from "rebootix/plugin-sdk";
 ...
 * for Rebootix. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: RebootixConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: RebootixConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: RebootixConfig): boolean => {
