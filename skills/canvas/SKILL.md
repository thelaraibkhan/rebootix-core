...
Display HTML content on connected Rebootix nodes (Mac app, iOS, Android).
...
```bash
cat ~/.rebootix/rebootix.json | jq '.gateway.bind'
```
...
1. Check server bind: `cat ~/.rebootix/rebootix.json | jq '.gateway.bind'`
...
Node is offline. Use `rebootix nodes list` to find online nodes.
