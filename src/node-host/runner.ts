...
const execHostEnforced = process.env.REBOOTIX_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.REBOOTIX_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureRebootixCliOnPath({ pathEnv: process.env.PATH ?? "" });
