...
function setSlackDmPolicy(cfg: RebootixConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "Rebootix";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for Rebootix`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: RebootixConfig, allowFrom: string[]): RebootixConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
