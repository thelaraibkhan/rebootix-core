...
function setWhatsAppDmPolicy(cfg: RebootixConfig, dmPolicy: DmPolicy): RebootixConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: RebootixConfig, allowFrom?: string[]): RebootixConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: RebootixConfig, selfChatMode: boolean): RebootixConfig {
...
async function detectWhatsAppLinked(cfg: RebootixConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so Rebootix can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for Rebootix" },
    ],
  });
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("rebootix channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
