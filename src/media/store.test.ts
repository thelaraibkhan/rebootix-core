...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "REBOOTIX_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "rebootix-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.REBOOTIX_STATE_DIR = path.join(home, ".rebootix");
    ...
    await fs.mkdir(path.join(home, ".rebootix"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.rebootix${path.sep}media`);
