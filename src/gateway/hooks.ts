...
export function resolveHooksConfig(cfg: RebootixConfig): HooksConfigResolved | null {
...
export function extractHookToken(req: IncomingMessage, url: URL): HookTokenResult {
  ...
  const headerToken =
    typeof req.headers["x-rebootix-token"] === "string"
      ? req.headers["x-rebootix-token"].trim()
      : "";
  if (headerToken) {
