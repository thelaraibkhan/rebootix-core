...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.REBOOTIX_SKIP_CHANNELS;
    prevSkipGmail = process.env.REBOOTIX_SKIP_GMAIL_WATCHER;
    process.env.REBOOTIX_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.REBOOTIX_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.REBOOTIX_SKIP_GMAIL_WATCHER;
    } else {
      process.env.REBOOTIX_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
