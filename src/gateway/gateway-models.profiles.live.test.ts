...
import { resolveRebootixAgentDir } from "../agents/agent-paths.js";
...
import { ensureRebootixModelsJson } from "../agents/models-config.js";
import { loadConfig } from "../config/config.js";
import type { RebootixConfig, ModelProviderConfig } from "../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.REBOOTIX_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.REBOOTIX_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.REBOOTIX_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<RebootixConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.REBOOTIX_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.REBOOTIX_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveRebootixAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "rebootix-live-state-"));
  ...
  process.env.REBOOTIX_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.rebootix-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveRebootixAgentDir();
  const sanitizedCfg: RebootixConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "Rebootix live tool probe (local, safe): " +
    ...
    process.env.REBOOTIX_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.REBOOTIX_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureRebootixModelsJson(cfg);
      ...
      const rawModels = process.env.REBOOTIX_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.rebootix-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
