...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.REBOOTIX_STATE_DIR;
  previousConfigPath = process.env.REBOOTIX_CONFIG_PATH;
  previousSkipBrowserControl = process.env.REBOOTIX_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.REBOOTIX_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.REBOOTIX_SKIP_CANVAS_HOST;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "rebootix-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.REBOOTIX_STATE_DIR = path.join(tempHome, ".rebootix");
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "rebootix-test-"))
    : path.join(tempHome, ".rebootix-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousConfigPath === undefined) {
      ...
      process.env.REBOOTIX_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.REBOOTIX_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.REBOOTIX_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.REBOOTIX_SKIP_GMAIL_WATCHER;
    } else {
      process.env.REBOOTIX_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.REBOOTIX_SKIP_CANVAS_HOST;
    } else {
      process.env.REBOOTIX_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.REBOOTIX_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.REBOOTIX_GATEWAY_TOKEN = fallbackToken;
  }
