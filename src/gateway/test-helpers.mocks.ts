...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `rebootix-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.REBOOTIX_CONFIG_PATH = path.join(root, "rebootix.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "rebootix.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "rebootix-gateway-test"),
        ...fileDefaults,
