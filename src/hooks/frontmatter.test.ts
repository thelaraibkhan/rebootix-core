...
`;
    ...
    expect(parsed.rebootix.emoji).toBe("💾");
    expect(parsed.rebootix.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.rebootix.emoji).toBe("📝");
    expect(parsed.rebootix.events).toEqual(["command"]);
    expect(parsed.rebootix.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.rebootix.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"rebootix": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"rebootix": {"events": ["test"]}}');
  });
...
describe("resolveRebootixMetadata", () => {
  it("extracts rebootix metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveRebootixMetadata(frontmatter);
  ...
  it("returns undefined when rebootix key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        rebootix: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Rebootix" },
            { id: "npm", kind: "npm", package: "@rebootix/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@rebootix/hook");
  });
    ...
    "rebootix":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Rebootix" }],
      },
  }
---
...
`;
    ...
    const rebootix = resolveRebootixMetadata(frontmatter);
    expect(rebootix).toBeDefined();
    expect(rebootix?.emoji).toBe("💾");
    expect(rebootix?.events).toEqual(["command:new"]);
    expect(rebootix?.requires?.config).toEqual(["workspace.dir"]);
    expect(rebootix?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const rebootix = resolveRebootixMetadata(frontmatter);
    expect(rebootix?.emoji).toBe("disk");
    expect(rebootix?.events).toEqual(["command:new"]);
  });
});
