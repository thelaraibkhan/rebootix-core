...
This directory contains hooks that ship with Rebootix. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs.rebootix.ai/hooks/soul-evil
...
Hooks can be configured in `~/.rebootix/rebootix.json`:
...
2. Restart gateway: `pkill -9 -f 'rebootix.*gateway' && pnpm rebootix gateway`
3. Enable the hook: `rebootix hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.rebootix.ai/hooks
