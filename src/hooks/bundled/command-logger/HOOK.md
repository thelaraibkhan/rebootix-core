...
homepage: https://docs.rebootix.ai/hooks#command-logger
metadata:
  {
    "rebootix":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.rebootix/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.rebootix/logs/commands.log ~/.rebootix/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/rebootix`:
   ```
   /home/username/.rebootix/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
