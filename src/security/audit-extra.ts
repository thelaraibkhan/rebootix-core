...
import type { RebootixConfig, ConfigFileSnapshot } from "../config/config.js";
...
function summarizeGroupPolicy(cfg: RebootixConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
export function collectAttackSurfaceSummaryFindings(cfg: RebootixConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep REBOOTIX_STATE_DIR on a local-only volume and re-run "${formatCliCommand("rebootix security audit --fix")}".`,
    });
  }
  return findings;
}
...
export function collectSecretsInConfigFindings(cfg: RebootixConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer REBOOTIX_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(cfg: RebootixConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectModels(cfg: RebootixConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
export function collectModelHygieneFindings(cfg: RebootixConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function hasWebSearchKey(cfg: RebootixConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: RebootixConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: RebootixConfig): boolean {
...
function isBrowserEnabled(cfg: RebootixConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: RebootixConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectExposureMatrixFindings(cfg: RebootixConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
