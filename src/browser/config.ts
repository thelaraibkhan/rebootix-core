import type { BrowserConfig, BrowserProfileConfig, RebootixConfig } from "../config/config.js";
import {
...
import {
  ...
  DEFAULT_REBOOTIX_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_REBOOTIX_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "rebootix" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_REBOOTIX_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_REBOOTIX_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Rebootix browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for rebootix/rebootix CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: RebootixConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_REBOOTIX_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "rebootix";
