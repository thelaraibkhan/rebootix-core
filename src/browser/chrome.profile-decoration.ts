...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".rebootix-profile-decorated");
}
 ...
 */
export function decorateRebootixProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_REBOOTIX_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_REBOOTIX_BROWSER_COLOR).toUpperCase();
