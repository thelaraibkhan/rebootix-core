...
vi.mock("./chrome.js", () => ({
  resolveRebootixUserDataDir: vi.fn(() => "/tmp/rebootix-test/rebootix/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveRebootixUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "rebootix-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveRebootixUserDataDir).mockReturnValue(userDataDir);
