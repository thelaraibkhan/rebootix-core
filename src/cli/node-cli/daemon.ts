...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("rebootix node install"), formatCliCommand("rebootix node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("rebootix node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    REBOOTIX_LOG_PREFIX: baseEnv.REBOOTIX_LOG_PREFIX ?? "node",
