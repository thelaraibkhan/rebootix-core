...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.rebootix${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.REBOOTIX_PROFILE = profile;

  const stateDir = env.REBOOTIX_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.REBOOTIX_STATE_DIR?.trim()) {
    env.REBOOTIX_STATE_DIR = stateDir;
  }

  if (!env.REBOOTIX_CONFIG_PATH?.trim()) {
    env.REBOOTIX_CONFIG_PATH = path.join(stateDir, "rebootix.json");
  }

  if (profile === "dev" && !env.REBOOTIX_GATEWAY_PORT?.trim()) {
    env.REBOOTIX_GATEWAY_PORT = "19001";
  }
}
