...
function appendRebootixHubHint(output: string, json?: boolean): string {
  if (json) {
    return output;
  }
...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("rebootix skills list")}\` to see all skills.`
      : "No skills found.";
    return appendRebootixHubHint(message, opts.json);
  }
  ...
  return appendRebootixHubHint(lines.join("\n"), opts.json);
}
...
export function formatSkillInfo(
  ...
  if (!skill) {
    ...
    return appendRebootixHubHint(
      `Skill "${skillName}" not found. Run \`${formatCliCommand("rebootix skills list")}\` to see available skills.`,
      opts.json,
    );
  }
...
export function registerSkillsCli(program: Command) {
  const skills = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/skills", "docs.rebootix.ai/cli/skills")}\n`,
    );
