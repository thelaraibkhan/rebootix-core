...
import { resolveRebootixPackageRoot } from "../infra/rebootix-root.js";
import {
  checkUpdateStatus,
...
const STEP_LABELS: Record<string, string> = {
  ...
  "rebootix doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "rebootix";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
const CLI_NAME = resolveCliName();
const REBOOTIX_REPO_URL = "https://github.com/rebootix/rebootix.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), ".rebootix");
...
function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("rebootix@")) {
    return trimmed.slice("rebootix@".length);
  }
...
function resolveGitInstallDir(): string {
  const override = process.env.REBOOTIX_GIT_DIR?.trim();
  if (override) {
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", REBOOTIX_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `REBOOTIX_GIT_DIR points at a non-git directory: ${params.dir}. Set REBOOTIX_GIT_DIR to an empty folder or an rebootix checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`REBOOTIX_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  const root =
    (await resolveRebootixPackageRoot({
  ...
  defaultRuntime.log(theme.heading("Rebootix update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating Rebootix..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this Rebootix install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("rebootix doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("rebootix gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g rebootix@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g rebootix@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.REBOOTIX_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.REBOOTIX_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("rebootix gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("rebootix doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("rebootix gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("rebootix gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `rebootix update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `REBOOTIX_GIT_DIR points at a non-git directory: ${gitDir}. Set REBOOTIX_GIT_DIR to an empty folder or an rebootix checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via REBOOTIX_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || !ok) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Rebootix to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["rebootix update", "Update a source checkout (git)"],
        ["rebootix update --channel beta", "Switch to beta channel (git + npm)"],
        ["rebootix update --channel dev", "Switch to dev channel (git + npm)"],
        ["rebootix update --tag beta", "One-off update to a dist-tag or version"],
        ["rebootix update --no-restart", "Update without restarting the service"],
        ["rebootix update --json", "Output result as JSON"],
        ["rebootix update --yes", "Non-interactive (accept downgrade prompts)"],
        ["rebootix update wizard", "Interactive update wizard"],
        ["rebootix --update", "Shorthand for rebootix update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run rebootix update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.rebootix.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["rebootix update status", "Show channel + version status."],
          ["rebootix update status --json", "JSON output."],
          ["rebootix update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.rebootix.ai/cli/update")}`,
    )
    .action(async (opts) => {
      try {
