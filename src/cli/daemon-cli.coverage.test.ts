...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    REBOOTIX_STATE_DIR: process.env.REBOOTIX_STATE_DIR,
    REBOOTIX_CONFIG_PATH: process.env.REBOOTIX_CONFIG_PATH,
    REBOOTIX_GATEWAY_PORT: process.env.REBOOTIX_GATEWAY_PORT,
    REBOOTIX_PROFILE: process.env.REBOOTIX_PROFILE,
  };

  beforeEach(() => {
    process.env.REBOOTIX_STATE_DIR = "/tmp/rebootix-cli-state";
    process.env.REBOOTIX_CONFIG_PATH = "/tmp/rebootix-cli-state/rebootix.json";
    delete process.env.REBOOTIX_GATEWAY_PORT;
    delete process.env.REBOOTIX_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.REBOOTIX_STATE_DIR !== undefined) {
      process.env.REBOOTIX_STATE_DIR = originalEnv.REBOOTIX_STATE_DIR;
    } else {
      delete process.env.REBOOTIX_STATE_DIR;
    }

    if (originalEnv.REBOOTIX_CONFIG_PATH !== undefined) {
      process.env.REBOOTIX_CONFIG_PATH = originalEnv.REBOOTIX_CONFIG_PATH;
    } else {
      delete process.env.REBOOTIX_CONFIG_PATH;
    }

    if (originalEnv.REBOOTIX_GATEWAY_PORT !== undefined) {
      process.env.REBOOTIX_GATEWAY_PORT = originalEnv.REBOOTIX_GATEWAY_PORT;
    } else {
      delete process.env.REBOOTIX_GATEWAY_PORT;
    }

    if (originalEnv.REBOOTIX_PROFILE !== undefined) {
      process.env.REBOOTIX_PROFILE = originalEnv.REBOOTIX_PROFILE;
    } else {
      delete process.env.REBOOTIX_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        REBOOTIX_PROFILE: "dev",
        REBOOTIX_STATE_DIR: "/tmp/rebootix-daemon-state",
        REBOOTIX_CONFIG_PATH: "/tmp/rebootix-daemon-state/rebootix.json",
        REBOOTIX_GATEWAY_PORT: "19001",
      },
