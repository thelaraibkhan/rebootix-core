...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "rebootix-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      REBOOTIX_NO_RESPAWN: "1",
      REBOOTIX_STATE_DIR: stateDir,
      REBOOTIX_SKIP_CHANNELS: "1",
      REBOOTIX_SKIP_GMAIL_WATCHER: "1",
      REBOOTIX_SKIP_CRON: "1",
      REBOOTIX_SKIP_BROWSER_CONTROL_SERVER: "1",
      REBOOTIX_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "rebootix-entry-bootstrap.mjs");
