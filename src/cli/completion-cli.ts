...
export async function installCompletion(shell: string, yes: boolean, binName = "rebootix") {
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c rebootix -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'rebootix gateway' and not 'rebootix other gateway', we need to check the sequence.
