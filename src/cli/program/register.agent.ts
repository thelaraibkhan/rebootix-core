...
${formatHelpExamples([
  ['rebootix agent --to +15555550123 --message "status update"', "Start a new session."],
  ['rebootix agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'rebootix agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'rebootix agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['rebootix agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.rebootix.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.rebootix.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['rebootix agents set-identity --agent main --name "Rebootix" --emoji "🦞"', "Set name + emoji."],
  ["rebootix agents set-identity --agent main --avatar avatars/rebootix.png", "Set avatar path."],
  [
    "rebootix agents set-identity --workspace ~/.rebootix/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "rebootix agents set-identity --identity-file ~/.rebootix/workspace/IDENTITY.md --agent main",
