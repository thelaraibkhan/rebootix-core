...
import { loadRebootixPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.REBOOTIX_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `rebootix-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "rebootix.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    delete process.env.REBOOTIX_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.REBOOTIX_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadRebootixPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.REBOOTIX_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadRebootixPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadRebootixPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("loads plugins from config paths", () => {
    process.env.REBOOTIX_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
