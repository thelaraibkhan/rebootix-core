...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `rebootix-plugin-install-${randomUUID()}`);
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.rebootix/extensions and uses unscoped id", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@rebootix/voice-call",
        version: "0.0.1",
        rebootix: { extensions: ["./dist/index.js"] },
      }),
      "utf-8",
    );
  ...
  it("rejects packages without rebootix.extensions", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
    const pkgDir = path.join(workDir, "package");
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@rebootix/nope", version: "0.0.1" }),
      "utf-8",
    );
    ...
    expect(result.error).toContain("rebootix.extensions");
  });
});
