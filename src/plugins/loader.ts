...
import { discoverRebootixPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: RebootixPluginDefinition;
  register?: RebootixPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as RebootixPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as RebootixPluginDefinition;
...
export function loadRebootixPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverRebootixPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "rebootix/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: RebootixPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as RebootixPluginModule;
    } catch (err) {
