...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.REBOOTIX_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.REBOOTIX_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.REBOOTIX_NODE_OPTIONS_READY = "1";
  ...
  child.once("error", (error) => {
    console.error(
      "[rebootix] Failed to respawn CLI:",
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[rebootix] ${parsed.error}`);
    process.exit(2);
  }
