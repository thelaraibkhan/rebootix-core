...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.REBOOTIX_GATEWAY_PORT).toBe("18789");
    expect(env.REBOOTIX_GATEWAY_TOKEN).toBe("secret");
    expect(env.REBOOTIX_SERVICE_MARKER).toBe("rebootix");
    expect(env.REBOOTIX_SERVICE_KIND).toBe("gateway");
    expect(typeof env.REBOOTIX_SERVICE_VERSION).toBe("string");
    expect(env.REBOOTIX_SYSTEMD_UNIT).toBe("rebootix-gateway.service");
    if (process.platform === "darwin") {
      expect(env.REBOOTIX_LAUNCHD_LABEL).toBe("ai.rebootix.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", REBOOTIX_PROFILE: "work" },
      port: 18789,
    });
    expect(env.REBOOTIX_SYSTEMD_UNIT).toBe("rebootix-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.REBOOTIX_LAUNCHD_LABEL).toBe("ai.rebootix.work");
    }
  });
});
