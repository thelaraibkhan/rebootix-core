...
export function buildServiceEnvironment(params: {
  ...
  const stateDir = env.REBOOTIX_STATE_DIR;
  const configPath = env.REBOOTIX_CONFIG_PATH;
  return {
    HOME: env.HOME,
    PATH: buildMinimalServicePath({ env }),
    REBOOTIX_PROFILE: profile,
    REBOOTIX_STATE_DIR: stateDir,
    REBOOTIX_CONFIG_PATH: configPath,
    REBOOTIX_GATEWAY_PORT: String(port),
    REBOOTIX_GATEWAY_TOKEN: token,
    REBOOTIX_LAUNCHD_LABEL: resolvedLaunchdLabel,
    REBOOTIX_SYSTEMD_UNIT: systemdUnit,
    REBOOTIX_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    REBOOTIX_SERVICE_KIND: GATEWAY_SERVICE_KIND,
