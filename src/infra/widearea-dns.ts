...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.REBOOTIX_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*rebootix-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "rebootix";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "rebootix");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "rebootix-gw");
  ...
  records.push(`_rebootix-gw._tcp IN PTR ${instanceLabel}._rebootix-gw._tcp`);
  records.push(`${instanceLabel}._rebootix-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._rebootix-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; rebootix-content-hash: ${contentHash}\n${contentBody}`;
}
