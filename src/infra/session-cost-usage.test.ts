...
describe("session cost usage", () => {
  it("aggregates daily totals with log cost and pricing fallback", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "rebootix-cost-"));
    ...
    const originalState = process.env.REBOOTIX_STATE_DIR;
    process.env.REBOOTIX_STATE_DIR = root;
    try {
      ...
      if (originalState === undefined) {
        ...
        process.env.REBOOTIX_STATE_DIR = originalState;
      }
    }
  });

  it("summarizes a single session file", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "rebootix-cost-session-"));
