...
function candidateBinDirs(opts: EnsureRebootixPathOpts): string[] {
  ...
  // Bundled macOS app: `rebootix` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "rebootix");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/rebootix` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "rebootix"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `rebootix` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureRebootixCliOnPath(opts: EnsureRebootixPathOpts = {}) {
  if (isTruthyEnvValue(process.env.REBOOTIX_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.REBOOTIX_PATH_BOOTSTRAPPED = "1";
