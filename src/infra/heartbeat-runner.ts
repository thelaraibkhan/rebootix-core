...
function resolveActiveHoursTimezone(cfg: RebootixConfig, raw?: string): string {
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: RebootixConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: RebootixConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: RebootixConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: RebootixConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: RebootixConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: RebootixConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: RebootixConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
