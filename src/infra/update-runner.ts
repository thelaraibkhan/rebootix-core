...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "rebootix-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "rebootix doctor",
        managerScriptArgs(manager, "rebootix", ["doctor", "--non-interactive"]),
        gitRoot,
        { REBOOTIX_UPDATE_IN_PROGRESS: "1" },
      ),
    );
