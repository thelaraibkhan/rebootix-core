...
export function triggerRebootixRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.REBOOTIX_SYSTEMD_UNIT,
        process.env.REBOOTIX_PROFILE,
      );
  ...
  const label =
    process.env.REBOOTIX_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.REBOOTIX_PROFILE);
