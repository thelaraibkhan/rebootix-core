...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "rebootix-commands-"));
...
function buildParams(commandBody: string, cfg: RebootixConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
