...
import { scheduleGatewaySigusr1Restart, triggerRebootixRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Rebootix in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerRebootixRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Rebootix via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
