import type { RebootixConfig } from "../config/types.js";
...
export function isCommandEnabled(cfg: RebootixConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: RebootixConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as RebootixConfig),
...
export function getCommandDetection(_cfg?: RebootixConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: RebootixConfig) {
