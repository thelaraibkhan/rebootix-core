...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "rebootix.sandbox=1");
  args.push("--label", `rebootix.sessionKey=${params.scopeKey}`);
  args.push("--label", `rebootix.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `rebootix.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("rebootix.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`rebootix sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`rebootix sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("rebootix sandbox recreate --all");
}
