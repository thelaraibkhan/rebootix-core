...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "rebootix-embedded-agent-"));
...
const ensureModels = (cfg: RebootixConfig) => ensureRebootixModelsJson(cfg, agentDir) as unknown;
