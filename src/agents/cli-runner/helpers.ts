...
/**
 * Cleanup suspended Rebootix CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: RebootixConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "rebootix-cli-images-"));
  const paths: string[] = [];
