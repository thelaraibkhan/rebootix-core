...
import { createRebootixCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("rebootix-ws-", async (workspaceDir) => {
      await withTempDir("rebootix-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createRebootixCodingTools({ workspaceDir });
    ...
    await withTempDir("rebootix-ws-", async (workspaceDir) => {
      await withTempDir("rebootix-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello rebootix");
        } finally {
          process.chdir(prevCwd);
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("rebootix-ws-", async (workspaceDir) => {
      await withTempDir("rebootix-override-", async (overrideDir) => {
        const tools = createRebootixCodingTools({ workspaceDir });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("rebootix-sandbox-", async (sandboxDir) => {
      await withTempDir("rebootix-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "rebootix-sbx-test",
        ...
        const tools = createRebootixCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
