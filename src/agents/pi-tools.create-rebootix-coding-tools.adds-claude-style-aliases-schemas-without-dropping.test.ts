...
import { createRebootixTools } from "./rebootix-tools.js";
import { __testing, createRebootixCodingTools } from "./pi-tools.js";
...
describe("createRebootixCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createRebootixTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createRebootixCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "rebootix-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "rebootix-outside.txt");
