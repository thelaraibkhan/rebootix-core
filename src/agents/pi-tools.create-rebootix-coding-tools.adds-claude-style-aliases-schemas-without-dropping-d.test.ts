...
const defaultTools = createRebootixCodingTools();

describe("createRebootixCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "rebootix-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createRebootixCodingTools();
    ...
    try {
      ...
      const contents = "Hello from rebootix read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "rebootix-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "rebootix-workspace"),
    ...
    const tools = createRebootixCodingTools({ sandbox });
