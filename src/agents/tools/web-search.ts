...
type WebSearchConfig = NonNullable<RebootixConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: RebootixConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("rebootix configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.rebootix.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://rebootix.ai",
      "X-Title": "Rebootix Web Search",
    },
    body: JSON.stringify({
      model: params.model,
      messages: [
