import type { RebootixConfig } from "./config.js";
import {
  getChatChannelMeta,
  listChatChannels,
...
function isTelegramConfigured(cfg: RebootixConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: RebootixConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: RebootixConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: RebootixConfig): boolean {
...
function isIMessageConfigured(cfg: RebootixConfig): boolean {
...
function isWhatsAppConfigured(cfg: RebootixConfig): boolean {
...
function isGenericChannelConfigured(cfg: RebootixConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: RebootixConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: RebootixConfig, providerId: string): boolean {
...
function isPluginExplicitlyDisabled(cfg: RebootixConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: RebootixConfig, pluginId: string): boolean {
...
function ensureAllowlisted(cfg: RebootixConfig, pluginId: string): RebootixConfig {
...
function enablePluginEntry(cfg: RebootixConfig, pluginId: string): RebootixConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
