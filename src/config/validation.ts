...
import type { RebootixConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: RebootixConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: RebootixConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = RebootixSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as RebootixConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as RebootixConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as RebootixConfig)),
    ),
  };
}
