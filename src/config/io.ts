...
import type { RebootixConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import { validateConfigObjectWithPlugins } from "./validation.js";
import { compareRebootixVersions } from "./version.js";
...
function coerceConfig(value: unknown): RebootixConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as RebootixConfig;
}
...
function stampConfigVersion(cfg: RebootixConfig): RebootixConfig {
...
function warnIfConfigFromFuture(cfg: RebootixConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareRebootixVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Rebootix (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: RebootixConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): RebootixConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as RebootixConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as RebootixConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: RebootixConfig) {
    clearConfigCache();
...
// module scope. `REBOOTIX_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.REBOOTIX_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.REBOOTIX_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): RebootixConfig {
...
export async function writeConfigFile(cfg: RebootixConfig): Promise<void> {
  clearConfigCache();
