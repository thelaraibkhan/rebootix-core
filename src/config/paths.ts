...
import type { RebootixConfig } from "./types.js";

/**
 * Nix mode detection: When REBOOTIX_NIX_MODE=1, the gateway is running under Nix.
 * In this mode:
...
export function resolveIsNixMode(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.REBOOTIX_NIX_MODE === "1";
}

export const isNixMode = resolveIsNixMode();

const LEGACY_STATE_DIRNAMES = [".rebootix", ".moltbot", ".moldbot"] as const;
const NEW_STATE_DIRNAME = ".rebootix";
const CONFIG_FILENAME = "rebootix.json";
const LEGACY_CONFIG_FILENAMES = ["rebootix.json", "moltbot.json", "moldbot.json"] as const;
 ...
 * Can be overridden via REBOOTIX_STATE_DIR.
 * Default: ~/.rebootix
 */
 ...
 * Can be overridden via REBOOTIX_CONFIG_PATH.
 * Default: ~/.rebootix/rebootix.json (or $REBOOTIX_STATE_DIR/rebootix.json)
 */
...
export function resolveConfigPath(
  ...
  const override = env.REBOOTIX_CONFIG_PATH?.trim();
  if (override) {
    return resolveUserPath(override);
  }
  const stateOverride = env.REBOOTIX_STATE_DIR?.trim();
  const candidates = [
 ...
 * Default: os.tmpdir()/rebootix-<uid> (uid suffix when available).
 */
export function resolveGatewayLockDir(tmpdir: () => string = os.tmpdir): string {
  const base = tmpdir();
  const uid = typeof process.getuid === "function" ? process.getuid() : undefined;
  const suffix = uid != null ? `rebootix-${uid}` : "rebootix";
 ...
 *
 * Precedence:
 * - `REBOOTIX_OAUTH_DIR` (explicit override)
...
export function resolveOAuthDir(
  ...
  const override = env.REBOOTIX_OAUTH_DIR?.trim();
  if (override) {
    return resolveUserPath(override);
  }
