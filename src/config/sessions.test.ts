...
describe("sessions", () => {
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-rebootix",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-rebootix#general");
  });
  ...
  it("updateLastRoute persists channel and target", async () => {
    ...
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "rebootix-sessions-"));
  ...
  it("derives session transcripts dir from REBOOTIX_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { REBOOTIX_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    const prev = process.env.REBOOTIX_STATE_DIR;
    process.env.REBOOTIX_STATE_DIR = "/custom/state";
    try {
      ...
      if (prev === undefined) {
        ...
        process.env.REBOOTIX_STATE_DIR = prev;
      }
    }
  });
