...
const ETC_REBOOTIX_DIR = path.join(ROOT_DIR, "etc", "rebootix");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "rebootix.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcRebootixPath(...parts: string[]) {
  return path.join(ETC_REBOOTIX_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcRebootixPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "rebootix.json"))).toEqual({
      shared: true,
    });
  });
});
