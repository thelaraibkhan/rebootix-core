...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveRebootixAgentDir: vi.fn().mockReturnValue("/tmp/rebootix-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/rebootix-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/rebootix-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveRebootixAgentDir: mocks.resolveRebootixAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveRebootixAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/rebootix-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/rebootix-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/rebootix-agent-custom");
