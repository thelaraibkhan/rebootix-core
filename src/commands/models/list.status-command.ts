...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveRebootixAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("rebootix models auth setup-token")}\` or \`${formatCliCommand("rebootix configure")}\`.`
          : `Run \`${formatCliCommand("rebootix configure")}\` or set an API key env var.`;
