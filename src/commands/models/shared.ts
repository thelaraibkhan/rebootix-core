...
export async function updateConfig(
  mutator: (cfg: RebootixConfig) => RebootixConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: RebootixConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: RebootixConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("rebootix agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
