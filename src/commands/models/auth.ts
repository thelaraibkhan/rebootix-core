...
import { readConfigFileSnapshot, type RebootixConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: RebootixConfig, model: string): RebootixConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("rebootix plugins install")}\`.`,
    );
  }
