...
export function listAgentEntries(cfg: RebootixConfig): AgentEntry[] {
...
function resolveAgentName(cfg: RebootixConfig, agentId: string) {
...
function resolveAgentModel(cfg: RebootixConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: RebootixConfig): AgentSummary[] {
