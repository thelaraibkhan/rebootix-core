...
import type { RebootixConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: RebootixConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("rebootix security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("rebootix configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("rebootix config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("rebootix doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("rebootix config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
