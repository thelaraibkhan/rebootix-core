...
beforeAll(() => {
  previousProfile = process.env.REBOOTIX_PROFILE;
  process.env.REBOOTIX_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.REBOOTIX_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/rebootix-root.js", () => ({
  resolveRebootixPackageRoot: vi.fn().mockResolvedValue("/tmp/rebootix"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/rebootix/pnpm-lock.yaml",
      markerPath: "/tmp/rebootix/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("Rebootix status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("rebootix status --all") ||
          l.includes("rebootix --profile isolated status --all") ||
          l.includes("rebootix status --all") ||
          l.includes("rebootix --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.REBOOTIX_GATEWAY_TOKEN = "abcd1234";
    try {
