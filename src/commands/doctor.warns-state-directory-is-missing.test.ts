...
beforeEach(() => {
  ...
  resolveRebootixPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.REBOOTIX_STATE_DIR;
  originalUpdateInProgress = process.env.REBOOTIX_UPDATE_IN_PROGRESS;
  process.env.REBOOTIX_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "rebootix-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.REBOOTIX_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.REBOOTIX_UPDATE_IN_PROGRESS;
  } else {
    process.env.REBOOTIX_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveRebootixPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadRebootixPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/rebootix-root.js", () => ({
  resolveRebootixPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "rebootix-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.REBOOTIX_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when REBOOTIX_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/rebootix.json",
      exists: true,
      raw: "{}",
    ...
    process.env.REBOOTIX_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
