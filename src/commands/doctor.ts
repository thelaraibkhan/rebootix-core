...
function resolveMode(cfg: RebootixConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("Rebootix doctor");

  const root = await resolveRebootixPackageRoot({
  ...
  let cfg: RebootixConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("rebootix configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("rebootix config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("rebootix setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("rebootix doctor --fix")}" to apply changes.`);
  }
