...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Rebootix status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("rebootix security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("rebootix security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.rebootix.ai/faq");
  runtime.log("Troubleshooting: https://docs.rebootix.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("rebootix status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("rebootix logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("rebootix status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("rebootix gateway probe")}`);
  }
}
