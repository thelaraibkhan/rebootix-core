...
const requireAgentDir = () => {
  const agentDir = process.env.REBOOTIX_AGENT_DIR;
  if (!agentDir) {
    throw new Error("REBOOTIX_AGENT_DIR not set");
  }
  return agentDir;
};
...
describe("writeOAuthCredentials", () => {
  ...
  afterEach(async () => {
    ...
    delete process.env.REBOOTIX_OAUTH_DIR;
  });

  it("writes auth-profiles.json under REBOOTIX_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "rebootix-oauth-"));
    process.env.REBOOTIX_STATE_DIR = tempStateDir;
    process.env.REBOOTIX_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.REBOOTIX_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to REBOOTIX_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "rebootix-minimax-"));
    process.env.REBOOTIX_STATE_DIR = tempStateDir;
    process.env.REBOOTIX_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.REBOOTIX_AGENT_DIR;
