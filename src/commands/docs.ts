...
const SEARCH_TOOL = "https://docs.rebootix.ai/mcp.SearchRebootix";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.rebootix.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('rebootix docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.rebootix.ai/");
      runtime.log(`Search: ${formatCliCommand('rebootix docs "your query"')}`);
    }
    return;
  }
