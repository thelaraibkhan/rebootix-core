...
describe("canvas host", () => {
  it("injects live reload script", () => {
    ...
    expect(out).toContain("rebootixCanvasA2UIAction");
    expect(out).toContain("rebootixSendUserAction");
  });

  it("creates a default index.html when missing", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "rebootix-canvas-"));
    ...
    try {
      ...
      expect(html).toContain("rebootixSendUserAction");
  ...
  it("serves the gateway-hosted A2UI scaffold", async () => {
    ...
    try {
      ...
      await fs.writeFile(bundlePath, "window.rebootixA2UI = {};", "utf8");
      createdBundle = true;
    }
    ...
    try {
      const res = await fetch(`http://127.0.0.1:${server.port}/__rebootix__/a2ui/`);
      const html = await res.text();
      expect(res.status).toBe(200);
      expect(html).toContain("rebootix-a2ui-host");
      expect(html).toContain("rebootixCanvasA2UIAction");

      const bundleRes = await fetch(
        `http://127.0.0.1:${server.port}/__rebootix__/a2ui/a2ui.bundle.js`,
      );
      const js = await bundleRes.text();
      expect(bundleRes.status).toBe(200);
      expect(js).toContain("rebootixA2UI");
    } finally {
      await server.close();
      if (createdBundle) {
