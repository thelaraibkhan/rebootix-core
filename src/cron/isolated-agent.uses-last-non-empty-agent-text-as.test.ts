...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "rebootix-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".rebootix", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<RebootixConfig> = {},
): RebootixConfig {
  const base: RebootixConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "rebootix"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".rebootix", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
