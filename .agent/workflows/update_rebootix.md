---
description: Update Rebootix from upstream when branch has diverged (ahead/behind)
---

# Rebootix Upstream Sync Workflow
...
```bash
# Kill running app
pkill -x "Rebootix" || true

# Move old version
mv /Applications/Rebootix.app /tmp/Rebootix-backup.app

# Install new build
cp -R dist/Rebootix.app /Applications/

# Launch
open /Applications/Rebootix.app
```
...
pnpm rebootix health

# Verify no zombie processes
ps aux | grep -E "(rebootix|gateway)" | grep -v grep

# Test agent functionality by sending a verification message
pnpm rebootix agent --message "Verification: macOS app rebuild successful - agent is responding." --session-id YOUR_TELEGRAM_SESSION_ID
...
# Update rebootix.json with fallback chains
...
pnpm rebootix agent --message "Verification: Upstream sync and macOS rebuild completed successfully." --session-id YOUR_TELEGRAM_SESSION_ID || echo "Warning: Agent test failed - check Telegram for verification message"
