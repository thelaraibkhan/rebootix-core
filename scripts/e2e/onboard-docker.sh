...
IMAGE_NAME="rebootix-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/rebootix-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/rebootix-bin/trash
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/rebootix-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/rebootix-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/rebootix-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$HOME/rebootix"
    config_path="$HOME/.rebootix/rebootix.json"
    sessions_dir="$HOME/.rebootix/agents/main/sessions"
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.rebootix/rebootix.json" <<'"'"'JSON'"'"'
{
