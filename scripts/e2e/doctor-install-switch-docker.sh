...
IMAGE_NAME="rebootix-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/rebootix-bin:$PATH"
  mkdir -p /tmp/rebootix-bin

  cat > /tmp/rebootix-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/rebootix-bin/systemctl

  cat > /tmp/rebootix-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/rebootix-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/rebootix"
  npm_entry="/tmp/npm-prefix/lib/node_modules/rebootix/rebootix.mjs"
  git_entry="/app/rebootix.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/rebootix-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/rebootix-gateway.service"
    if [ ! -f "$unit_path" ]; then
