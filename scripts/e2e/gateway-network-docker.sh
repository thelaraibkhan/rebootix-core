...
IMAGE_NAME="rebootix-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="rebootix-net-e2e-$$"
GW_NAME="rebootix-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "REBOOTIX_GATEWAY_TOKEN=$TOKEN" \
	  -e "REBOOTIX_SKIP_CHANNELS=1" \
	  -e "REBOOTIX_SKIP_GMAIL_WATCHER=1" \
	  -e "REBOOTIX_SKIP_CRON=1" \
	  -e "REBOOTIX_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
