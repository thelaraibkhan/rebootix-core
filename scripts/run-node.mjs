...
const logRunner = (message) => {
  if (env.REBOOTIX_RUNNER_LOG === "0") {
    return;
  }
  process.stderr.write(`[rebootix] ${message}\n`);
};

const runNode = () => {
  const nodeProcess = spawn(process.execPath, ["rebootix.mjs", ...args], {
    cwd,
    env,
    stdio: "inherit",
  });
