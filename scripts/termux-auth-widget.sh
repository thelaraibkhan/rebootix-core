...
# Rebootix Auth Widget for Termux
...
# Check auth status
termux-toast "Checking Rebootix auth..."

STATUS=$(ssh "$SERVER" '$HOME/rebootix/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/rebootix/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|REBOOTIX_EXPIRING|REBOOTIX_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/rebootix/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|REBOOTIX_EXPIRED|REBOOTIX_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Rebootix won't work until re-auth"
                ;;
        esac
        ;;
