...
BASE_IMAGE="${BASE_IMAGE:-rebootix-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-rebootix-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=rebootix.sandbox=1)
NOTE
