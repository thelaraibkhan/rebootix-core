#!/bin/bash
# Setup Rebootix Auth Management System
...
echo "=== Rebootix Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'rebootix-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Rebootix message: Send warning via Rebootix itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/rebootix-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/rebootix-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/rebootix-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now rebootix-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status rebootix-auth-monitor.timer"
echo ""
echo "Quick commands:"
