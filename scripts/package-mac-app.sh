...
# Build and bundle Rebootix into a minimal .app we can open.
# Outputs to dist/Rebootix.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Rebootix.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Rebootix"
BUNDLE_ID="${BUNDLE_ID:-ai.rebootix.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/rebootix/rebootix/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Rebootix/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :RebootixBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :RebootixGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Rebootix"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Rebootix"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Rebootix"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Rebootix" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Rebootix/Resources/Rebootix.icns" "$APP_ROOT/Contents/Resources/Rebootix.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Rebootix/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying RebootixKit resources"
REBOOTIXKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/RebootixKit_RebootixKit.bundle"
if [ -d "$REBOOTIXKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/RebootixKit_RebootixKit.bundle"
  cp -R "$REBOOTIXKIT_BUNDLE" "$APP_ROOT/Contents/Resources/RebootixKit_RebootixKit.bundle"
else
  echo "WARN: RebootixKit resource bundle not found at $REBOOTIXKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Rebootix"
killall -q Rebootix 2>/dev/null || true
