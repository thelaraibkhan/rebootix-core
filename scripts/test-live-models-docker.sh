...
docker run --rm -t \
  ...
  -e REBOOTIX_LIVE_MODELS="${REBOOTIX_LIVE_MODELS:-${REBOOTIX_LIVE_MODELS:-all}}" \
  -e REBOOTIX_LIVE_PROVIDERS="${REBOOTIX_LIVE_PROVIDERS:-${REBOOTIX_LIVE_PROVIDERS:-}}" \
  -e REBOOTIX_LIVE_MODEL_TIMEOUT_MS="${REBOOTIX_LIVE_MODEL_TIMEOUT_MS:-${REBOOTIX_LIVE_MODEL_TIMEOUT_MS:-}}" \
  -e REBOOTIX_LIVE_REQUIRE_PROFILE_KEYS="${REBOOTIX_LIVE_REQUIRE_PROFILE_KEYS:-${REBOOTIX_LIVE_REQUIRE_PROFILE_KEYS:-}}" \
