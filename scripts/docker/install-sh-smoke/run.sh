...
INSTALL_URL="${REBOOTIX_INSTALL_URL:-https://rebootix.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${REBOOTIX_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${REBOOTIX_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="rebootix"
PACKAGE_NAME="${REBOOTIX_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (REBOOTIX_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${REBOOTIX_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${REBOOTIX_INSTALL_LATEST_OUT:-}"
fi
