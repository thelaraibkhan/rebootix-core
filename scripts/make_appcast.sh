...
ZIP=${1:?"Usage: $0 Rebootix-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/rebootix/rebootix/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Rebootix-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/rebootix/rebootix/releases/download/v${VERSION}/"}
