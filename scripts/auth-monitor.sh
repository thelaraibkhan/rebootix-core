...
#
# Suggested cron: */30 * * * * /home/admin/rebootix/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Rebootix notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., rebootix-alerts)
...
STATE_FILE="$HOME/.rebootix/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Rebootix if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use rebootix
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Rebootix to $NOTIFY_PHONE..."
            rebootix send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Rebootix is down. Run: ssh l36 '~/rebootix/scripts/mobile-reauth.sh'" "urgent"
    exit 1
