...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: RebootixProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.rebootix", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: RebootixKit.AnyCodable]()) {
                $0[$1.key] = RebootixKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: RebootixProtocol.AnyCodable?) -> [String: RebootixProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: RebootixProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: RebootixKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: RebootixProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("rebootix.control.heartbeat")
    static let controlAgentEvent = Notification.Name("rebootix.control.agent")
}
