...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.rebootix", category: "gateway.env")
    private static let supportedBindModes: Set<String> = ["loopback", "tailnet", "lan", "auto"]

    static func gatewayPort() -> Int {
        if let raw = ProcessInfo.processInfo.environment["REBOOTIX_GATEWAY_PORT"] {
            let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
            if let parsed = Int(trimmed), parsed > 0 { return parsed }
        }
        if let configPort = RebootixConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.rebootixExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "rebootix CLI not found in PATH; install the CLI.")
            }
    ...
    private static func preferredGatewayBind() -> String? {
        ...
        if let env = ProcessInfo.processInfo.environment["REBOOTIX_GATEWAY_BIND"] {
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "rebootix@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "rebootix@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "rebootix@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "rebootix@\(target)"])
            }

        statusHandler("Installing rebootix@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed rebootix@\(target)")
        } else {
