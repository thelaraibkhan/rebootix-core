...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: RebootixOAuthStore.AnthropicOAuthStatus = RebootixOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: RebootixOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = RebootixOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = RebootixOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try RebootixOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. Rebootix can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: RebootixOAuthStore.AnthropicOAuthStatus,
