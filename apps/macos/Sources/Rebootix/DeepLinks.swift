...
private let deepLinkLogger = Logger(subsystem: "ai.rebootix", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "Rebootix is paused", message: "Unpause Rebootix to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run Rebootix agent?", message: body) else { return }
        }
