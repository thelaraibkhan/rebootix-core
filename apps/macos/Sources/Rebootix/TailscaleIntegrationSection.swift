...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.rebootix/rebootix.json. Prefer REBOOTIX_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.rebootix/rebootix.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.rebootix/rebootix.json. Restart the gateway to apply."
        }
