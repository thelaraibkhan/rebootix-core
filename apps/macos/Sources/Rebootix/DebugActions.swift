...
enum DebugActions {
    private static let verboseDefaultsKey = "rebootix.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "rebootix.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = RebootixPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Rebootix", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = RebootixPaths.configURL
        guard
