...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.rebootix", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: RebootixPeekabooBridgeServices?
    private static var rebootixSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("Rebootix", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = RebootixPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.rebootixSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.rebootixSocketPath, privacy: .public)")
    }
...
@MainActor
private final class RebootixPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.rebootix.peekaboo")
