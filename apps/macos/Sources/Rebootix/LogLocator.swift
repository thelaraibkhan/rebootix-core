...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["REBOOTIX_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/rebootix")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("rebootix-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("rebootix-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/rebootix/ (rolling or stdout), or nil if none exist.
