...
enum CommandResolver {
    private static let projectRootDefaultsKey = "rebootix.gatewayProjectRootPath"
    private static let helperName = "rebootix"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let rebootixEntry = root.appendingPathComponent("rebootix.mjs").path
        if FileManager().isReadableFile(atPath: rebootixEntry) { return rebootixEntry }
        let binEntry = root.appendingPathComponent("bin/rebootix.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/rebootix")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let rebootixPaths = self.rebootixManagedPaths(home: home)
        if !rebootixPaths.isEmpty {
            extras.insert(contentsOf: rebootixPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + rebootixPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func rebootixManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".rebootix"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func rebootixExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectRebootixExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("rebootix.mjs").path,
            root.appendingPathComponent("bin/rebootix.js").path,
        ]
    ...
    static func hasAnyRebootixInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.rebootixExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func rebootixNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let rebootixPath = self.projectRebootixExecutable(projectRoot: root) {
                return [rebootixPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "rebootix", subcommand] + extraArgs
            }
            if let rebootixPath = self.rebootixExecutable(searchPaths: searchPaths) {
                return [rebootixPath, subcommand] + extraArgs
            }

            let missingEntry = """
            rebootix entrypoint missing (looked for dist/index.js or rebootix.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real rebootix CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/rebootix"
        ...
        \(cliSection)
        \(projectSection)
        if command -v rebootix >/dev/null 2>&1; then
          CLI="$(command -v rebootix)"
          rebootix \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/rebootix.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/rebootix.mjs"
            node "$PRJ/rebootix.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/rebootix.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/rebootix.js"
            node "$PRJ/bin/rebootix.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent rebootix"
          pnpm --silent rebootix \(quotedArgs);
        else
          echo "rebootix CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? RebootixConfigFile.loadDict()
