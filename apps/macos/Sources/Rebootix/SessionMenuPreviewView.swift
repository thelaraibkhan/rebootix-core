...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.rebootix", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> RebootixSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: RebootixSessionPreviewEntry,
    ...
    private static func cache(payload: RebootixSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: RebootixChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [RebootixKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [RebootixKit.AnyCodable]) -> [RebootixChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(RebootixChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: RebootixChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: RebootixChatMessage) -> Bool {
    ...
    private static func toolNames(for message: RebootixChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: RebootixChatMessage) -> String? {
