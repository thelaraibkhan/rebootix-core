...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (Rebootix token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: RebootixOAuthStore.AnthropicOAuthStatus = RebootixOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.rebootix", category: "anthropic-oauth")
...
enum RebootixOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let rebootixOAuthDirEnv = "REBOOTIX_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "Rebootix OAuth token file not found"
            case .unreadableFile: "Rebootix OAuth token file not readable"
            case .invalidJSON: "Rebootix OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in Rebootix OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "Rebootix OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.rebootixOAuthDirEnv]?
