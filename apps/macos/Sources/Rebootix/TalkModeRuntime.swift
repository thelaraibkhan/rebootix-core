...
actor TalkModeRuntime {
    ...
    private let logger = Logger(subsystem: "ai.rebootix", category: "talk.runtime")
    private let ttsLogger = Logger(subsystem: "ai.rebootix", category: "talk.tts")
    ...
    private func latestAssistantText(sessionKey: String, since: Double? = nil) async -> String? {
        do {
            ...
            let decoded: [RebootixChatMessage] = messages.compactMap { item in
