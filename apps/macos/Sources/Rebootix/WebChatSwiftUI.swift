...
private let webChatSwiftLogger = Logger(subsystem: "ai.rebootix", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: RebootixChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> RebootixChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(RebootixChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> RebootixChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                RebootixGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    RebootixAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<RebootixChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any RebootixChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = RebootixChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: RebootixChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<RebootixChatView>) -> NSViewController
    {
