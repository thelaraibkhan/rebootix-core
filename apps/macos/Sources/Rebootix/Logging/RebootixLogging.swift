...
enum RebootixLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = RebootixOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = RebootixFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        RebootixLogging.bootstrapIfNeeded()
        let label = RebootixLogging.makeLabel(subsystem: subsystem, category: category)
...
struct RebootixOSLogHandler: LogHandler {
...
struct RebootixFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = RebootixLogging.parseLabel(self.label)
