...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: RebootixProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: RebootixProtocol.AnyCodable] = [
            "presence": .init([RebootixProtocol.AnyCodable(entry)]),
        ]
        let payload = RebootixProtocol.AnyCodable(payloadMap)
