...
import Foundation
import Testing
@testable import RebootixChatUI
...
private final class TestChatTransport: @unchecked Sendable, RebootixChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [RebootixChatHistoryPayload]
    private let sessionsResponses: [RebootixChatSessionsListResponse]

    private let stream: AsyncStream<RebootixChatTransportEvent>
    private let continuation: AsyncStream<RebootixChatTransportEvent>.Continuation

    init(
        historyResponses: [RebootixChatHistoryPayload],
        sessionsResponses: [RebootixChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<RebootixChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> RebootixChatHistoryPayload {
        ...
        return self.historyResponses.last ?? RebootixChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [RebootixChatAttachmentPayload]) async throws -> RebootixChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return RebootixChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> RebootixChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? RebootixChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: RebootixChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = RebootixChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = RebootixChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { RebootixChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = RebootixChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = RebootixChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = RebootixChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { RebootixChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
