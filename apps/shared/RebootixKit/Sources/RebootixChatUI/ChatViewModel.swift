...
private let chatUILogger = Logger(subsystem: "ai.rebootix", category: "RebootixChatUI")

@MainActor
@Observable
public final class RebootixChatViewModel {
    public private(set) var messages: [RebootixChatMessage] = []
    ...
    public var attachments: [RebootixPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [RebootixChatPendingToolCall] = []
    public private(set) var sessions: [RebootixChatSessionEntry] = []
    private let transport: any RebootixChatTransport
    ...
    private var pendingToolCallsById: [String: RebootixChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any RebootixChatTransport) {
    ...
    public var sessionChoices: [RebootixChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [RebootixChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [RebootixChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: RebootixPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [RebootixChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: RebootixChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [RebootixChatMessage]) -> [RebootixChatMessage] {
        var result: [RebootixChatMessage] = []
    ...
    private static func dedupeKey(for message: RebootixChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [RebootixChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> RebootixChatAttachmentPayload in
            RebootixChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> RebootixChatSessionEntry {
        RebootixChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: RebootixChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: RebootixChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: RebootixAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = RebootixChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> RebootixPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
