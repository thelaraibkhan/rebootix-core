...
struct ChatMessageBubble: View {
    let message: RebootixChatMessage
    let style: RebootixChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? RebootixChatTheme.userText : RebootixChatTheme.assistantText
    ...
    private var inlineAttachments: [RebootixChatMessageContent] {
    ...
    private var toolCalls: [RebootixChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [RebootixChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? RebootixChatTheme.userBubble
        }
        if self.style == .onboarding {
            return RebootixChatTheme.onboardingAssistantBubble
        }
        return RebootixChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return RebootixChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: RebootixChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? RebootixChatTheme.userText : RebootixChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: RebootixChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(RebootixChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(RebootixChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [RebootixChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: RebootixChatTheme.assistantText)
            }
        }
    }
}
