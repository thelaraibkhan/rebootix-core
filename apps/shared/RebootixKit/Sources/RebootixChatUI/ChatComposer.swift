...
@MainActor
struct RebootixChatComposer: View {
    @Bindable var viewModel: RebootixChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(RebootixChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(RebootixChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \RebootixPendingAttachment.id)
                { (att: RebootixPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            RebootixPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Rectangle()
                .fill(RebootixChatTheme.divider)
                .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(RebootixChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(RebootixChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(RebootixChatTheme.subtleCard)
        .clipShape(Capsule())
    }
