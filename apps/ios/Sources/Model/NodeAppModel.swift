...
final class NodeAppModel {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = RebootixCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = RebootixCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = RebootixCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = RebootixCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = RebootixCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    private func resolveA2UIHostURL() async -> String? {
        ...
        return base.appendingPathComponent("__rebootix__/a2ui/").absoluteString + "?platform=ios"
    }
    ...
    func requestLocationPermissions(mode: RebootixLocationMode) async -> Bool {
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for rebootix:// links.)
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            switch command {
            case RebootixLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case RebootixCanvasCommand.present.rawValue,
                 RebootixCanvasCommand.hide.rawValue,
                 RebootixCanvasCommand.navigate.rawValue,
                 RebootixCanvasCommand.evalJS.rawValue,
                 RebootixCanvasCommand.snapshot.rawValue:
                return try await self.handleCanvasInvoke(req)
            case RebootixCanvasA2UICommand.reset.rawValue,
                 RebootixCanvasA2UICommand.push.rawValue,
                 RebootixCanvasA2UICommand.pushJSONL.rawValue:
                return try await self.handleCanvasA2UIInvoke(req)
            case RebootixCameraCommand.list.rawValue,
                 RebootixCameraCommand.snap.rawValue,
                 RebootixCameraCommand.clip.rawValue:
                return try await self.handleCameraInvoke(req)
            case RebootixScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            default:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: RebootixNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: RebootixNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(RebootixLocationGetParams.self, from: req.paramsJSON)) ??
            RebootixLocationGetParams()
        ...
        let payload = RebootixLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case RebootixCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(RebootixCanvasPresentParams.self, from: req.paramsJSON)) ??
                RebootixCanvasPresentParams()
        ...
        case RebootixCanvasCommand.hide.rawValue:
            return BridgeInvokeResponse(id: req.id, ok: true)
        case RebootixCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(RebootixCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case RebootixCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(RebootixCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case RebootixCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(RebootixCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case RebootixCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              ...
              if (!host) return JSON.stringify({ ok: false, error: "missing rebootixA2UI" });
              return JSON.stringify(host.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case RebootixCanvasA2UICommand.push.rawValue, RebootixCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [AnyCodable]
            if command == RebootixCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(RebootixCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try RebootixCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(RebootixCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try RebootixCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.rebootixA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case RebootixCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case RebootixCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(RebootixCameraSnapParams.self, from: req.paramsJSON)) ??
                RebootixCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case RebootixCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(RebootixCameraClipParams.self, from: req.paramsJSON)) ??
                RebootixCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(RebootixScreenRecordParams.self, from: req.paramsJSON)) ??
            RebootixScreenRecordParams()
...
private extension NodeAppModel {
    func locationMode() -> RebootixLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return RebootixLocationMode(rawValue: raw) ?? .off
    }
