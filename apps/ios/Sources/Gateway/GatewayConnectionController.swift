...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [RebootixCapability.canvas.rawValue, RebootixCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(RebootixCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(RebootixCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = RebootixLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(RebootixCapability.location.rawValue) }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            RebootixSystemCommand.notify.rawValue,
            RebootixSystemCommand.which.rawValue,
            RebootixSystemCommand.run.rawValue,
            RebootixSystemCommand.execApprovalsGet.rawValue,
            RebootixSystemCommand.execApprovalsSet.rawValue,
        ]

        let caps = Set(self.currentCaps())
        if caps.contains(RebootixCapability.camera.rawValue) {
            commands.append(RebootixCameraCommand.list.rawValue)
            commands.append(RebootixCameraCommand.snap.rawValue)
            commands.append(RebootixCameraCommand.clip.rawValue)
        }
        if caps.contains(RebootixCapability.location.rawValue) {
            commands.append(RebootixLocationCommand.get.rawValue)
        }
